...
<p align="center">
  <img src="https://raw.githubusercontent.com/vynaa9-create/Agen-clipboard/refs/heads/main/src/Screenshot_2026-05-31-20-08-37-62.png" alt="NeuroClip Install Preview" width="80%" />
</p>
...
```bash
pkg update -y && pkg install nodejs termux-api git unzip -y
...
```bash
pkg update -y && pkg install nodejs termux-api unzip -y
termux-setup-storage
cp /sdcard/Download/Agen-clipboard-fixed.zip ~/
cd ~ && unzip -o Agen-clipboard-fixed.zip -d Agen-clipboard
cd Agen-clipboard
bash scripts/install-fresh.sh
neuro on
```
...
- **Termux** — dari F-Droid atau GitHub Release
- **Termux:API** — **WAJIB**, tanpa ini notifikasi dan clipboard tidak bisa jalan. Install dari F-Droid, GitHub Release, atau toko aplikasi yang sama dengan Termux kamu. Jangan campur sumber install antara Termux dan Termux:API (misalnya Termux dari F-Droid tapi Termux:API dari Play Store).
- **Termux:Widget** — opsional, untuk shortcut layar utama
- **MacroDroid** — opsional, untuk otomasi trigger

> ⚠️ **Penting:** Termux:API harus diinstall dari sumber yang sama dengan Termux. Jika Termux dari F-Droid, maka Termux:API juga harus dari F-Droid. Campur sumber bisa menyebabkan error permission atau notifikasi tidak muncul.
